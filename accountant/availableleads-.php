          ...
          <div class="col-lg-10 order-lg-0 order-1">
            ...
            <div class="row g-4">
               <?php 
                    ...
                    if (isset($_GET['search']) AND !empty($_GET['search']) ) {
                          ...
                          LEFT JOIN companytype ON company.tid=companytype.tid
                           WHERE job.JSts = 1 AND job.endDate >= CURDATE() AND job.Jtitle LIKE '%$searchValue%' OR job.Jsalary='$searchValue' OR job.Jcity='$searchValue' OR job.distanceKm='$searchValue' OR job.Jcredits='$searchValue' ORDER BY Jid DESC ";
                           // print_r($query);die;
                                                    // echo "<script> alert('ok')</script>";

                       }else{
                        $querysett="SELECT * FROM `setting`
                    WHERE Aid='$id'";
                    $setting = mysqli_query($conn,$querysett);
                    $check1 = mysqli_fetch_array($setting);
                    if (!empty($check1)) {
                      $queryset="SELECT * FROM `setting`
                    WHERE Aid='$id'";
              ...
              <div>
                  ...
                  <p class="pt-2">
                  <?php echo substr($row['JdesBef'],0,70) ?>
                   </p>
                   <p>
                   ...
                   <p class="mb-3" id="hidePara">
                      <?php echo substr($row['JdesBef'],70,2000) ?>
                   </p>
                            ...
                            setInterval(function(){
                              ...
                              var endTime = new Date(`${date} ${month} ${year}`);
                                ...
                                if(days < 0 && hours < 0 && minutes < 0 && seconds < 0){
                                  $(".countdown-"+id).html(`POST EXPIRE`)
                                }

                                $(".countdown-"+id).html(`${days} Days ${hours} Hours ${minutes} Minutes ${seconds} Seconds`)
                            },1000);
                          </script>
                          </span>
                      </p>
