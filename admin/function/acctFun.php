<?php  
        ...
        if (isset($_POST["save"])) {
                ...
                if ($ext == 'jpg' OR $ext == 'jpeg' OR $ext == 'png' OR $ext == 'gif' OR $ext == 'JPG' OR $ext == 'JPEG' OR $ext == 'PNG' OR $ext == 'GIF') 
                {
                        ...
                        {
                                $sql = ("INSERT INTO `accountant`(`Aname`, `Aphone`,`surname`, `Aemail`, `Aaddress`, `kvknumber`, `admId`,
                                        `ASts`,`cid` ,`Ai_mg`,`Acity`, `pwd_hash`, `pwd_salt`) VALUES ('$name','$contact','$surname','$email','$address','$kvk', '$admid', 1,
                                        '$cid','$upload_directory','$city', '$strHash', '$strSalt')");
  ...
  if (isset($_POST["editstaff"])) {
    ...
    $nAdmin = mysqli_real_escape_string($conn,$_POST['admin']);
    // $$upload_directory= "uploadedImages/IMG_0040.JPG";
    // $longPath="../accountant/uploadedImages/"; 
    // $upload_directory    = "uploadedImages/";     // define path  
    //     $filename            = $_FILES["simage"]["name"];
    ...
    //     $ext                 = pathinfo($filename,PATHINFO_EXTENSION);
        ...
        `kvknumber`='$kvk', `Acity`='$city', `admId`='$nAdmin' WHERE Aid='$id'";
